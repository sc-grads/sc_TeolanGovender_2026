...
go
create proc AverageBalance(@EmpNumberFrom int, @EmpNumberTo int, @AverageBalance int OUTPUT) as
begin
	SET NOCOUNT ON
	declare @TotalAmount decimal(5,2)
	declare @NumOfEmployee int
	begin try
		print 'The employee numbers are from ' + convert(varchar(10),@EmpNumberFrom) 
		      + ' to ' + convert(varchar(10),@EmpNumberTo)
		select @TotalAmount = sum(Amount) from tblTransaction
		where EmpNumber between @EmpNumberFrom and @EmpNumberTo
		select @NumOfEmployee = count(distinct EmpNumber) from tblEmployee
		where EmpNumber between @EmpNumberFrom and @EmpNumberTo
...
GO
DECLARE @AvgBalance int, @ReturnStatus int
exec @ReturnStatus = AverageBalance @EmpNumberFrom = 323, @EmpNumberTo = 327, @AverageBalance = @AvgBalance OUTPUT
select @AvgBalance as Average_Balance, @ReturnStatus as Return_Status
